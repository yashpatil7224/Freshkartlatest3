...
SENDER_EMAIL = os.getenv("SENDER_EMAIL", "patilyash948@gmail.com")
SENDER_PASSWORD = os.getenv("SENDER_PASSWORD", "ouzj nhco sscr fipn")  # Paste 16-character Gmail App Password here

def send_email_otp(to_email: str, otp_code: str) -> bool:
    """
    Sends 6-digit OTP verification email to user from yashpatil7224@gmail.com using Gmail SMTP.
    """
    ...
    if not pass_clean or pass_clean == "YOUR_16_DIGIT_GMAIL_APP_PASSWORD":
        print("[NOTICE] SENDER_PASSWORD (Gmail App Password) is not set in main.py.")
        print("[NOTICE] To deliver live emails to real inboxes, paste your 16-character Gmail App Password on Line 266 of main.py.")
        return False
    ...
    try:
        ...
        try:
            with smtplib.SMTP("smtp.gmail.com", 587, timeout=10) as server:
                server.starttls()
                server.login(SENDER_EMAIL, pass_clean)
                server.sendmail(SENDER_EMAIL, [to_email], msg.as_string())
        except Exception as tls_err:
            print(f"[SMTP TLS Fallback] Port 587 failed ({tls_err}), trying SSL Port 465...")
            with smtplib.SMTP_SSL("smtp.gmail.com", 465, timeout=10) as server:
                server.login(SENDER_EMAIL, pass_clean)
                server.sendmail(SENDER_EMAIL, [to_email], msg.as_string())

        print(f"[SUCCESS] Email OTP successfully sent to {to_email} via Gmail SMTP from {SENDER_EMAIL}!")
        return True
    except Exception as e:
        print(f"[SMTP ERROR] Failed to send email via Gmail SMTP: {e}")
        print(f"[SMTP HELP] Check Google Account settings or Spam folder.")
        return False
...
def send_order_delivery_email(to_email: str, order_id: str, date_str: str, customer_info: dict, items: list, total_amount: float) -> bool:
    ...
    try:
        msg = MIMEMultipart()
        msg["Subject"] = f"Order Delivered #{order_id} - FreshKart Tax Invoice Attached"
        msg["From"] = SENDER_EMAIL
        msg["To"] = to_email
        msg.attach(MIMEText(html_body, "html"))
        ...
        if pdf_bytes:
            ...
            pdf_attachment.add_header("Content-Disposition", "attachment", filename=f"FreshKart_Invoice_{order_id}.pdf")
            msg.attach(pdf_attachment)

        if not pass_clean or pass_clean == "YOUR_16_DIGIT_GMAIL_APP_PASSWORD":
            print(f"[ORDER DELIVERED EMAIL NOTICE] Simulated send for Order #{order_id} to {to_email}.")
            return False
        ...
        print(f"[SUCCESS] Order Delivered Email + PDF Bill attached sent to {to_email} for Order #{order_id}!")
        return True
    except Exception as e:
        print(f"[ORDER DELIVERED EMAIL ERROR] Failed to send email: {e}")
        return False
...
def send_admin_order_email_alert(order_id: str, customer_info: dict, items: list, total_amount: float) -> bool:
    ...
    try:
        msg = MIMEMultipart()
        msg["Subject"] = f"NEW ORDER RECEIVED Order #{order_id} - Rs. {total_amount:.2f} ({c_name})"
        msg["From"] = SENDER_EMAIL
        msg["To"] = ADMIN_EMAIL
        msg.attach(MIMEText(html_body, "html"))
        ...
        print(f"[SUCCESS] Admin Order Alert Email sent to {ADMIN_EMAIL} for Order #{order_id}!")
        return True
    except Exception as e:
        print(f"[ADMIN EMAIL ALERT LOG] {e}")
        return False
...
def forgot_password_send_otp(payload: schemas.ForgotPasswordOTPRequest, db: Session = Depends(get_db)):
    ...
    # Verify account exists with this email
    ...
    target_email = existing_user.email or email_clean
